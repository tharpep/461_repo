...
def calculate_net_score(model_id: str) -> Dict:
    ...
    print(f"License Score: {license_score:.3f} (latency: {license_latency:.3f}s)")
    
    # Ramp Up Time Score (0.2 weight)
    ramp_up_score, ramp_up_latency = ramp_up_time_score(model_id)
    print(f"Ramp Up Score: {ramp_up_score:.3f} (latency: {ramp_up_latency:.3f}s)")
    ...
    print(f"Dataset & Code Score: {dataset_code_score:.3f} (latency: {dataset_code_latency:.3f}s)")
    
    # Dataset Quality Score (0.15 weight)
    dataset_quality, dataset_quality_latency = dataset_quality_sub_score(model_id)
    print(f"Dataset Quality Score: {dataset_quality:.3f} (latency: {dataset_quality_latency:.3f}s)")
    
    # Code Quality Score (0.1 weight) - Not implemented, using default
    code_quality = 0.5  # Default value since no code quality scoring function exists
    code_quality_latency = 0
    print(f"Code Quality Score: {code_quality:.3f} (default - not implemented)")
    
    # Performance Claims Score (0.1 weight)
    performance_claims, performance_claims_latency = performance_claims_sub_score(model_id)
    print(f"Performance Claims Score: {performance_claims:.3f} (latency: {performance_claims_latency:.3f}s)")
