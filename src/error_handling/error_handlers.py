...
def retry_on_error(
    ...
    exceptions: tuple = (NetworkError, requests.exceptions.RequestException)
):
    """
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                raise HuggingFaceError(
                    ...
                    severity=ErrorSeverity.MEDIUM,
                    original_error=e,
                    recovery_suggestion="Check model ID and network connectivity"
                )
...
def create_error_summary(errors: list) -> dict:
    ...
    return {
        ...
        "highest_severity": max(severity_breakdown.keys()) if severity_breakdown else None
    }
